...
function mkFormat(inspectOptions) {
    var formatFn = util.format;
    if (!formatFn || typeof inspectOptions === 'object') {

        // If node < 0.6, then use its `util.format`:
        // <https://github.com/joyent/node/blob/master/lib/util.js#L22>:
        var inspect = util.inspect;
        var formatRegExp = /%[sdj%]/g;
        formatFn = function format(f) {
...
function Logger(options, _childOptions, _childSimple) {
    ...
    if (parent) {
        this._level = parent._level;
        this._inspectDepth = parent._inspectDepth;
        this.streams = [];
    ...
    if (options.inspectDepth) {
        this._inspectDepth = options.inspectDepth;
    }
    this._format = mkFormat(this._inspectDepth && {depth: this._inspectDepth});
...
Logger.prototype.addSerializers = function addSerializers(serializers) {
    ...
    Object.keys(serializers).forEach(function (field) {
        ...
        if (typeof (serializer) !== 'function') {
            throw new TypeError(this._format(
...
Logger.prototype.levels = function levels(name, value) {
    ...
    if (typeof (name) === 'number') {
        ...
        if (!stream) {
            throw new Error(this._format('no stream with name "%s"', name));
        }
    }
    if (value === undefined) {
        return stream.level;
...
Logger.prototype._applySerializers = function (fields, excludeFields) {
    ...
    Object.keys(this.serializers).forEach(function (name) {
        ...
        try {
            ...
            _warn(self._format('bunyan: ERROR: Exception thrown from the "%s" '
                + 'Bunyan serializer. This should never happen. This is a bug'
                + 'in that serializer function.\n%s',
                name, err.stack || err));
            fields[name] = self._format('(Error in Bunyan log "%s" serializer '
        ...
        try {
            ...
            if (safeJsonStringify) {
                ...
                _warn('bunyan: ERROR: Exception in '
                    ...
                    + _indent(this._format('%s\n%s', util.inspect(rec), e.stack)),
                    dedupKey);
                str = this._format('(Exception in JSON.stringify(rec): %j. '
    ...
    return function () {
        ...
        if (! this._emit) {
            ...
            if (!_haveWarned[dedupKey]) {
                ...
                _warn(this._format('bunyan usage error: %s:%s: attempt to log '
